...
class PartnerNewApiService(Component):
    ...
    @restapi.method(
        [(["/<int:id>/get", "/<int:id>"], "GET")], output_param=Datamodel("partner.info"), auth="public",
    )
    def get(self, _id):
        """
        ...
        partner_info.state = self.env.datamodels["state.info"](id=partner.state_id.id, name=partner.state_id.name)
