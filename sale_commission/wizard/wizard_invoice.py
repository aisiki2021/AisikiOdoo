...
class SaleCommissionMakeInvoice(models.TransientModel):
    ...
    company_id = fields.Many2one(comodel_name="res.company", related="journal_id.company_id", readonly=True)
    product_id = fields.Many2one(string="Product for invoicing", comodel_name="product.product", required=True)
    settlement_ids = fields.Many2many(
        ...
        domain="[('state', '=', 'settled'),('agent_type', '=', 'agent')," "('company_id', '=', company_id)]",
    ...
    def button_create(self):
        ...
        invoices = settlements.make_invoices(self.journal_id, self.product_id, date=self.date)
        # go to results
        if len(settlements):
            return {
